...
async def main():
    ...
    for selected_task in selected_tasks:
        try:
            selected_task = int(selected_task.strip())  # Convert to integer and remove spaces
            if selected_task in task_functions:
                ...
                task_fields = required_fields_for_process.get(function_name, [])
    ...
    patient_details['family_name'] = args.family_name if args.family_name else (input("Enter Family Name: ") if 'family_name' in required_fields else None)
    patient_details['given_name'] = args.given_name if args.given_name else (input("Enter Given Name: ") if 'given_name' in required_fields else None)
    patient_details['dob'] = args.dob if args.dob else (input("Enter DOB (DDMMYYYY): ") if 'dob' in required_fields else None)
    patient_details['medicare_number'] = str(args.medicare_number) if args.medicare_number else (str(input("Enter Medicare Number: ")) if 'medicare_number' in required_fields else None)
    if 'sex' in required_fields:
    # Get the value from args if provided, otherwise prompt for input
        patient_details['sex'] = args.sex.upper() if args.sex else input("Enter Sex (M, F, or I): ").upper()
    ...
    for task_number in selected_tasks:
        try:
            ...
            if task_number in task_functions:
                task = asyncio.create_task(task_functions[task_number](patient_details, shared_state))
