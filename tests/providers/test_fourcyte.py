...
class TestFourCyte(PlaywrightTestCase):
    ...
    async def test_login(self, initialized_session, test_credentials):
        ...
        popup_page.get_by_placeholder = MagicMock(
        ...
        popup_page.get_by_role = MagicMock(
        ...
        page.get_by_label = MagicMock(
            side_effect=lambda label: {"Access results portal": access_button}.get(
                label, Mock()
            )
        )
        ...
        password_field.fill.assert_called_with(
            test_credentials["4cyte"]["user_password"]
        )
