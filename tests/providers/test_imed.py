...
class TestIMed(PlaywrightTestCase):
    ...
    async def test_login(self, initialized_session, test_credentials):
        ...
        popup_page.locator = MagicMock(side_effect=lambda selector: {
        ...
        popup_page.get_by_test_id = MagicMock(side_effect=lambda test_id: {
        ...
        page.get_by_test_id = MagicMock(side_effect=lambda test_id: {
            "dropdownInput": postcode_input
        }.get(test_id, Mock()))
        ...
        page.get_by_role.assert_any_call("button", name=test_credentials["IMed"]["suburb"])
        ...
        popup_page.locator.assert_any_call('[data-testid="SingleLineTextInputField-FormControl"][name="uid"]')
        username_field.fill.assert_called_with(test_credentials["IMed"]["user_name"])
        
        popup_page.locator.assert_any_call('[data-testid="SingleLineTextInputField-FormControl"][name="password"]')
        password_field.fill.assert_called_with(test_credentials["IMed"]["user_password"])
    ...
    async def test_search_patient(self, initialized_session, test_patient):
        ...
        page.locator.assert_called_with('[data-testid="SingleLineTextInputField-FormControl"][name="nameOrPatientId"]')
        name_field.fill.assert_called_with(f'{test_patient["given_name"]} {test_patient["family_name"]}')
        
        # Verify DOB entry
        page.get_by_test_id.assert_any_call("DOB-input-field-form-control")
        dob_field.click.assert_called_once()
        dob_field.type.assert_called_with(convert_date_format(test_patient["dob"], "%d%m%Y", "%d/%m/%Y"))
