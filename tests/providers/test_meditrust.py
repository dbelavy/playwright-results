...
class TestMediTrust(PlaywrightTestCase):
    ...
    async def test_login(self, initialized_session, test_credentials):
        ...
        page.get_by_placeholder = MagicMock(
            side_effect=lambda text: {"Authentication Code": code_field}.get(
                text, Mock()
            )
        )
    ...
    @pytest.mark.asyncio
    async def test_create_without_patient(
        self, mock_page, mock_browser, test_credentials
    ):
