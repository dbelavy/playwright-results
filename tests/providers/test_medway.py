...
class TestMedway(PlaywrightTestCase):
    ...
    async def test_login(self, initialized_session, test_credentials):
        ...
        self.setup_page_elements(
            page,
            label_elements={"Username": username_element, "Password": password_element},
            role_elements={("button", "Log in"): login_button},
        )
        ...
        username_element.fill.assert_called_with(
            test_credentials["Medway"]["user_name"]
        )

        page.get_by_label.assert_any_call("Password")
        password_element.fill.assert_called_with(
            test_credentials["Medway"]["user_password"]
        )
