...
class TestMyHealthRecord(PlaywrightTestCase):
    ...
    async def test_login(self, initialized_session, test_credentials):
        ...
        page.get_by_label = MagicMock(side_effect=lambda label, **kwargs: {
        ...
        password_field.fill.assert_called_with(test_credentials["PRODA"]["user_password"])
        ...
        page.click.assert_any_call(f'input[name="radio1"][value="{test_credentials["PRODA"]["PRODA_full_name"]}"]')
    ...
    async def test_search_patient(self, initialized_session, test_patient):
        ...
        page.get_by_label = MagicMock(side_effect=lambda label, **kwargs: {
            ...
            "Not Stated": gender_radio if test_patient["sex"] not in ["M", "F", "I"] else Mock(),
