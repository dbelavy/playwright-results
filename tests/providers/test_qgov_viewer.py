...
class TestQGovViewer(PlaywrightTestCase):
    ...
    async def test_login(self, initialized_session, test_credentials):
        ...
        page.get_by_placeholder = MagicMock(
            side_effect=lambda text: {"Your email address": email_field}.get(
                text, Mock()
            )
        )

        page.get_by_label = MagicMock(
            side_effect=lambda label: {"Password": password_field}.get(label, Mock())
        )
        ...
        password_field.fill.assert_called_with(
            test_credentials["QGov"]["user_password"]
        )
    ...
    async def test_search_patient(self, initialized_session, test_patient):
        ...
        page.locator = MagicMock(
            side_effect=lambda selector: {"#MedicareNumber": medicare_field}.get(
                selector, Mock()
            )
        )
        ...
        gender_select.select_option.assert_called_with(
            convert_gender(test_patient["sex"], "M1F2I3")
        )
