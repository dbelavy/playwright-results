...
class TestQScan(PlaywrightTestCase):
    ...
    async def test_login(self, initialized_session, test_credentials):
        ...
        password_field.fill.assert_called_with(test_credentials["QScan"]["user_password"])
    ...
    async def test_search_patient(self, initialized_session, test_patient):
        ...
        page.locator.assert_any_call("a.btn.portalButton.selfServeButton[title='Access restricted studies']")
        ...
        page.keyboard.type.assert_any_call(f"{test_patient['family_name']},{test_patient['given_name']}")
        ...
        page.locator.assert_any_call("div.gwt-HTML:text('A patient that matches your search criteria was found:')")
    ...
    async def test_search_patient_not_found(self, initialized_session, test_patient):
        ...
        page.locator.assert_any_call("div.gwt-HTML:text('No patient that matches your search criteria was found.')")
        
        # Verify no access button click
        assert not any(call[0][0] == "button.gwt-Button.accessButton" for call in page.locator.call_args_list)
