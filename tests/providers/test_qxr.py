...
class TestQXR(PlaywrightTestCase):
    ...
    async def test_search_patient(self, initialized_session, test_patient):
        ...
        page.locator = MagicMock(
            side_effect=lambda selector: {"div.arrow.arrow-up": arrow_locator}.get(
                selector, Mock()
            )
        )
        ...
        page.get_by_role = MagicMock(
            side_effect=lambda role: {"button": button_mock}.get(role, Mock())
        )
        ...
        search_field.fill.assert_called_with(
    ...
    @pytest.mark.asyncio
    async def test_search_patient_error_handling(
        self, initialized_session, test_patient
    ):
        ...
        # Mock placeholder selectors
        page.get_by_placeholder = MagicMock(
            side_effect=lambda text: {"Search patient name, id,": search_field}.get(
                text, Mock()
            )
        )
